...
//상품별 할인 적용 후 총 가격 계산
function calculateProductDiscountedPrice(cartList: CartItem[]): number {
  return cartList.reduce((sum, item) => {
    const productRate =
      item.count >= DISCOUNT_POLICY.PRODUCT.MIN_COUNT ? DISCOUNT_POLICY.PRODUCT.RATE[item.id] || 0 : 0;
    return sum + item.price * item.count * (1 - productRate);
  }, 0);
}

// BULK 할인률 계산
function calculateBulkDiscountRate(totalPrice: number, productDiscountedPrice: number, totalItemCount: number): number {
...
// 날짜(화요일) 세일 할인율 적용
function calculateDateSaleDiscountRate(currentRate: number): number {
...
// 메인 할인 계산기
export function calculateDiscountRate(totalPrice: number, cartList: CartItem[]) {
